...
| Native lines with real contextual data | §2 above | [`00-native-lines`](examples/00-native-lines/) |
| Native quantitative filled areas | §2 above | [`00-native-choropleth`](examples/00-native-choropleth/) |
...
- **Sample data keeps its source semantics and licence.** Airports and Natural Earth boundaries are public domain; the cable snapshot is OpenStreetMap/ODbL; World Bank GDP is CC BY 4.0; trajectories are synthesised at runtime. Missing and incomplete data stay explicit. See [`docs/data-sources.md`](docs/data-sources.md), and keep that honesty if you adapt an example.

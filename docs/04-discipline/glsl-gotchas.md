...
1. `map.isStyleLoaded()` can read `false` at mount time and never become the trigger you were waiting for — the same one-shot `load`/`style.load` problem covered in [4.3](shared-gl-context.md).
...
- [4.2 Depth and blending](depth-and-blending.md) — more silent failures in the same render path, one layer up
- [4.3 Sharing a WebGL context](shared-gl-context.md) — the `style.load` / `isStyleLoaded()` trap this page's `addImage` section leans on
- [3.3 Vector field particles](../03-scaling-up/vector-field-particles.md) — the fat-line technique above, at scale

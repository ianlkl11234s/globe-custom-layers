...
These fixture runs upgrade examples 09 and 10 from `unverified` to `reproduced`. They do not cover a non-fixture antimeridian orbit, a Polygon hole or MultiPolygon in real WebGL, observed remove/re-add GPU cleanup, pixel alignment, production visitor-token entry, or an authoritative ADIZ source. The Taiwan-area data remains an illustrative fixture and `vertical boundary walls` remains the component name.

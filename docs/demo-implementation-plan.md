...
The website now presents eight reusable globe elements: native point, line and area layers first, followed by glow points, great-circle arcs, mass trajectories, satellite orbital rings and vertical boundary walls. The supporting fixtures are 1,174 OurAirports locations, 281 OSM-derived North Atlantic cable LineStrings, 50 European map units joined to World Bank 2023 GDP, and explicitly synthetic or schematic route/track/orbit/ADIZ geometry. The repository contains 13 standalone Mapbox examples and 327 tests across the twelve examples with test suites; the effects gallery has no unit suite.

Local acceptance for the current branch includes 20 site tests, site typecheck/build, 13 satellite tests, 20 boundary-wall tests, both affected example typechecks/builds, documentation checks, whitespace checks, and browser readback of all eight token-free MapLibre scenes. The new generic orbit and boundary renderers still need a real-token Mapbox browser gate. The currently published English-default atlas is deployed at [globe-custom-layers.zeabur.app](https://globe-custom-layers.zeabur.app/); branch changes require a separate release readback.

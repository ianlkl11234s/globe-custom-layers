...
Decision recorded 2026-09-05 and updated through 2026-09-06. This is a chronological implementation and verification record. Historical counts and “no commit” statements below describe the checkpoint where they were written; the consolidated batch was merged through [PR #1](https://github.com/ianlkl11234s/globe-custom-layers/pull/1), the generic orbit/wall and Agent-contract work through [PR #7](https://github.com/ianlkl11234s/globe-custom-layers/pull/7), and the WebGL2 shader closeout through [PR #8](https://github.com/ianlkl11234s/globe-custom-layers/pull/8) as `1a084fe`. These releases passed post-merge CI and production readback within the boundaries recorded below.

## Current atlas state — 2026-09-06

The website now presents eight reusable globe elements: native point, line and area layers first, followed by glow points, great-circle arcs, mass trajectories, satellite orbital rings and vertical boundary walls. The supporting fixtures are 1,174 OurAirports locations, 281 OSM-derived North Atlantic cable LineStrings, 50 European map units joined to World Bank 2023 GDP, and explicitly synthetic or schematic route/track/orbit/ADIZ geometry. The repository contains 13 standalone Mapbox examples and 328 tests across the twelve examples with test suites; the effects gallery has no unit suite.

Local acceptance includes 20 site tests, site typecheck/build, 13 satellite tests, 21 boundary-wall tests, both affected example typechecks/builds, documentation checks, whitespace checks, browser readback of all eight token-free MapLibre scenes, and real-token Mapbox runs for the bundled orbit and boundary-wall fixtures. The remaining renderer gates are non-fixture antimeridian/hole appearance, observed remove/re-add GPU cleanup, and quantitative alignment rather than basic tile access or shader compilation. The currently published English-default atlas is deployed at [globe-custom-layers.zeabur.app](https://globe-custom-layers.zeabur.app/); every new merge still needs its own CI and production readback.

## Product decision

Keep Globe Custom Layers a **Mapbox-first rendering cookbook for developers and AI agents**. The website is its visual entry point: choose an effect, inspect it, adjust a few parameters, then open its recipe/source or copy a task prompt. The selected MapLibre site adapters are locally reproduced, while direct ECEF/mainMatrix and terrain/depth remain unverified; the Mercator gallery is separate inspiration. This project does not become a flight-data backend, events platform, or npm rendering framework.
...
## Satellite-orbit and vertical-boundary-wall additions — 2026-09-06

Two new standalone Mapbox-first examples were added from read-only visual references in `satellite-art` and `plan-art`: `09-satellite-orbits` renders three closed, inclined and elevated schematic orbital rings with moving markers; `10-adiz-walls` provides a reusable vertical-boundary-wall component, demonstrated with a locally authored Taiwan-area five-corner ADIZ schematic. Neither copies a live feed or claims authoritative operational data. The orbit values are not TLE propagation, and the ADIZ outline is not an AIP, sovereign-airspace statement or legal boundary; its 500 km wall is display height, not a ceiling.

Each new example passed `npm ci`, TypeScript, nine focused geometry/runtime-token bridge tests and a production build. The integrated site passed 20 Node tests, TypeScript, token-neutral static build, documentation checks and whitespace checks. Browser acceptance on the token-free MapLibre build showed all three satellite rings and moving markers, plus the five-face vertical boundary wall using the ADIZ fixture, with no console warning/error. A 390×844 viewport retained one canvas and `scrollWidth === innerWidth`; the initial eight-item four-column rail overflow was corrected to a readable three-column layout. Real-token Mapbox service rendering remains unverified, so both manifest entries retain `unverified` status and their SVGs are explicitly labelled illustrations rather than runtime screenshots. No commit, push or deployment was performed.

## Generic component and Agent-contract correction — 2026-09-06

The two additions were rebuilt as injected components rather than renamed demos. `createOrbitLayer(orbits, controls)` now consumes validated closed paths whose renderer-boundary altitude is metres; display scaling no longer mutates simulation state, Mercator positions keep a continuous world copy at the antimeridian, and high-altitude points use a camera-ray/sphere visibility test instead of surface-normal culling. `createVerticalBoundaryWallLayer(data, controls)` now consumes GeoJSON-like Polygon or MultiPolygon data, emits geodesically subdivided side walls for exterior and interior rings without a top, keeps antimeridian copies continuous, rejects invalid geometry, and disposes renderer-owned resources. Taiwan ADIZ remains one explicitly schematic fixture; the component and public scene name are `vertical boundary walls`.

`examples/manifest.json` version 2 is now the technical source for the eight website scenes, component/fixture language, accepted input and units, complete source sets, and acceptance criteria. The site build generates `sceneCatalog.js` from this array instead of maintaining a second prompt catalogue. Generated prompts require a target repository and map lifecycle owner, data count/update rate, interactions, source commit, full source set, licence/provenance, explicit missing/error behavior, and separate test/type/build/WebGL/data-readback/production evidence. A copied fixture must be replaced at the application boundary; target data belongs in its own module and must not be embedded in the renderer.

The current branch is split into four implementation commits before the documentation closeout: `cb375c7` makes the manifest authoritative for prompts, `e6c2d31` rebuilds vertical walls, `622bc5b` rebuilds satellite orbits, and `206d0f5` integrates the two MapLibre scenes and controls. Local acceptance passed 13 satellite tests, 20 wall tests, 20 site tests, all affected TypeScript checks and builds, the documentation checker, and whitespace checks. Browser readback on the token-free build showed both scene names and fixture labels, three orbit features, altitude scale `1× → 2×`, speed `1× → 0.5×`, pause/resume, and wall display height `500 km → 1,000 km`. It also showed the final Agent prompt's full source-set and separate application-data-module requirements. The existing Vite large-chunk and runtime-resolved MapLibre CSS warnings remain advisory; they were not treated as passing performance evidence.

A fixed cold-start reproduction used program commit `206d0f551b4d6d6ee5a747c8c4d1791477646e11`, `gpt-5.6-terra` at medium reasoning, the manifest contracts, the complete standalone examples, no network data, no token, and isolated output under `/private/tmp/globe-agent-eval-terra-206d0f5`. It produced two non-fixture orbit paths including an antimeridian crossing and a non-ADIZ MultiPolygon with a hole and antimeridian crossing. Main-agent reruns passed 14 orbit tests and 22 wall tests; both copies also passed TypeScript and production builds. Two additional bounded cold-start runs independently produced separate application data modules and passed 14 orbit and 21 wall tests.

The evaluation found two instruction defects instead of hiding them. Its first selector described `siteScenes` as keyed properties even though it is an array; the guide now says to select by `exampleId`. The fixed Terra run injected through the correct generic APIs but reused the fixture modules for application data, so the generated prompt and guide now explicitly require a separate application-owned data module and an unambiguous complete source set. These generated copies prove that an Agent can adapt the contracts into compiling, tested standalone applications. They do not prove Mapbox service access, WebGL shader compilation, browser globe/Mercator behavior, visual antimeridian continuity, GPU cleanup, deployment, or production readback; those cells remain open.

## Real Mapbox renderer closeout — 2026-09-06

A local root `.env` supplied `VITE_MAPBOX_TOKEN` to examples 09 and 10 through a temporary Vite `envDir` config. The token was not copied into either example or committed, and this record does not repeat it; provider requests and browser developer tools can still observe a public token. Mapbox GL JS 3.30.0 loaded its real style and tiles for both runs.

The satellite fixture rendered three elevated inclined rings and moving markers. Altitude changed from 1.4× to 2.5×, speed changed from 1.0× to 0.5×, paused captures were identical, playback changed the next capture, and the high-zoom flat endpoint kept a continuous orbit line. The browser reported no warning or error.

The first wall run failed at actual GPU compilation: `flat` was used as a vertex-shader variable even though GLSL ES 3.0 reserves it as an interpolation qualifier. Renaming it to `mercatorPosition` restored the open-top walls. The follow-up run showed the Taiwan-area fixture at 280 km and 600 km display heights, retained the walls at the high-zoom flat endpoint, and added no warning or error after reload. A unit guard now rejects selected GLSL ES 3.0 qualifier names as declared shader identifiers.

Merge commit `1a084fe` passed the post-merge workflow across every example and the site, then received a successful Zeabur production deployment. Public readback found the updated `reproduced` statuses and the fixed `mercatorPosition` shader in the served bundle. The production atlas session-token flow mounted one iframe at a time and rendered both fixtures with real Mapbox tiles; the UI reported `Mapbox · WebGL` and the browser had no warning or error.

These fixture runs upgrade examples 09 and 10 from `unverified` to `reproduced`. They do not cover a non-fixture antimeridian orbit, a Polygon hole or MultiPolygon in real WebGL, observed remove/re-add GPU cleanup, quantitative pixel alignment, or an authoritative ADIZ source. The Taiwan-area data remains an illustrative fixture and `vertical boundary walls` remains the component name.

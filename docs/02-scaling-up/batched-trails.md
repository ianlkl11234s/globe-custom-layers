...
The obvious implementation scans the map of currently-occupied slots for the minimum `endTime`. That scan is O(capacity) per acquisition. At scale that's ruinous: when demand outstrips capacity by a wide margin, most acquisitions in a given frame *are* evictions, so an O(capacity) scan runs thousands of times per frame — measured at world scale as roughly 92% of this layer's own per-frame cost, tens of millions of iterations in a single frame. [2.4](debugging-performance.md) walks through that exact incident end to end.
...
- [2.4 Finding the actual bottleneck](debugging-performance.md) — the case study where this eviction scan was measured, not guessed at
- [2.2 Instanced tracks](instanced-tracks.md) — the companion pattern for point/instance data instead of line geometry
...
- [3.2 Depth and blending](../03-discipline/depth-and-blending.md) — render order and blending for a mesh like this one

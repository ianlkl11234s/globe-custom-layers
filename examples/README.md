...
| [`10-adiz-walls`](10-adiz-walls/) | An injectable vertical polygon-boundary-wall component for exteriors and holes, demonstrated with a Taiwan ADIZ schematic fixture. | [1.1 Mapbox](../docs/01-hugging-the-globe/mapbox.md) | 20 |

Every one of them passes `npx tsc --noEmit` and `npm run build`; all but the gallery also carry unit tests, **327 in total**. The satellite suite covers injection, closed paths, timing, display scaling, dateline copies, high-altitude visibility, validation and runtime-token bridging. The wall suite covers injected Polygon/MultiPolygon data, holes, geodesic densification, antimeridian continuity, validation, cleanup and runtime-token bridging. Website tests are counted separately. These checks run without a browser, a GPU, or a token; live interactions remain a separate browser gate.

...
| [`09-satellite-orbits`](09-satellite-orbits/) | An injectable satellite-orbit component with elevated closed paths, inclination and moving markers, demonstrated with three schematic definitions. | [1.1 Mapbox](../docs/01-hugging-the-globe/mapbox.md) | 13 |
| [`10-adiz-walls`](10-adiz-walls/) | An injectable vertical polygon-boundary-wall component for exteriors and holes, demonstrated with a Taiwan ADIZ schematic fixture. | [1.1 Mapbox](../docs/01-hugging-the-globe/mapbox.md) | 21 |

Every one of them passes `npx tsc --noEmit` and `npm run build`; all but the gallery also carry unit tests, **328 in total**. The satellite suite covers injection, closed paths, timing, display scaling, dateline copies, high-altitude visibility, validation and runtime-token bridging. The wall suite covers injected Polygon/MultiPolygon data, holes, geodesic densification, antimeridian continuity, validation, shader identifier safety, cleanup and runtime-token bridging. Website tests are counted separately. These checks run without a browser, a GPU, or a token; live interactions remain a separate browser gate.

The original nine have historical real-token browser evidence. The satellite-orbit and vertical-boundary-wall fixtures were reproduced locally against real Mapbox tiles on 2026-09-06; their READMEs state which non-fixture and lifecycle cases remain unit-only. The two newer native line/area examples still have build evidence without a real-token browser run. Read each status marker rather than assuming.

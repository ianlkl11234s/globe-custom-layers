...
export interface GlowLayerControls {
   ...
   * this frame, plus (new in this example) the camera's ECEF position this
   * frame -- `null` whenever it isn't meaningful (mercator mode, or globe
   * mode before Mapbox has handed back a `cameraMerc` at least once). This
   * is not just HUD decoration here: main.ts's screen-space picking strategy
   * reads `cameraEcef` at click time to run its own backface test.
   */
  onFrameInfo?: (info: {
    isGlobe: boolean;
    transition: number;
    cameraEcef: { x: number; y: number; z: number } | null;
  }) => void;
}
 ...
 * this file is copied unmodified): `onFrameInfo` now also reports `cameraEcef`
 * (see `GlowPointsScene.getCameraEcef()`), so examples/07-picking-and-popups'
 * screen-space picking strategy can do its own backface test using the exact
 * same camera-ECEF value this layer's shader already computed for its visual
 * cull -- see this file's `render()` for where that value comes from.
 */
export function createGlowLayer(controls: GlowLayerControls): CustomLayerInterface {
  ...
  return {
    ...
    render(
      ...
      scene.render(matrix);
      controls.onFrameInfo?.({ isGlobe, transition, cameraEcef: scene.getCameraEcef() });

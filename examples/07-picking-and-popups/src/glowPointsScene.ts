 ...
 *
 * MODIFIED here vs. examples/00-native-vs-custom / 01-points-on-globe (where
 * this file is copied unmodified): added `cameraEcefValid` + `getCameraEcef()`
 * so examples/07-picking-and-popups' screen-space picking strategy can reuse
 * this scene's own camera-ECEF computation for its backface test instead of
 * recomputing it independently. Drawing logic below is untouched.
 */
...
export class GlowPointsScene {
  ...
  // Set true only when uCameraEcef holds a real, freshly-computed camera
  // position (globe active AND a cameraMerc was supplied this call or a
  // previous one). getCameraEcef() below reads this instead of guessing from
  // uCameraEcef alone, because (0,0,0) is BOTH the "never set" default AND a
  // theoretically reachable real value -- the flag is the only unambiguous
  // signal. Added for examples/07-picking-and-popups' screen-space picking
  // strategy, which needs the exact same camera-ECEF this scene's own
  // backface cull uses (see this file's module docstring "Modified for
  // 07" note, and glowLayer.ts's onFrameInfo).
  ...
  setGlobe(
    ...
    if (!globeToMerc || globeToMerc.length < 16) {
      ...
      this.cameraEcefValid = false; // no globe -> "behind the globe" is meaningless
      return;
    }
  ...
  /**
   * Returns this frame's camera position in ECEF space, or `null` when it is
   * not meaningful (mercator mode, or globe mode before the first real
   * `cameraMerc` has ever arrived).
   *
   * Added for examples/07-picking-and-popups: screen-space picking needs a
   * backface test in ECEF space, and this accessor lets it reuse the *exact*
   * camera-ECEF value the vertex shader's own backface cull (GLOBE_PROJECT_GLSL)
   * already computed for this frame, via the uCameraEcef uniform -- rather
   * than re-deriving it from map.getFreeCameraOptions() a second time. Two
   * independent derivations of the same quantity is exactly the kind of thing
   * that quietly drifts apart after the next edit; reading the one the shader
   * already trusts removes that risk entirely.
   */
  getCameraEcef(): { x: number; y: number; z: number } | null {
    if (!this.material || !this.cameraEcefValid) return null;
    const v = this.material.uniforms.uCameraEcef!.value as THREE.Vector3;

// Sidebar cards + toolbar. All shared state lives on the `state` object;
// each effect's `_group` / `_params` are assigned by layer.js.
...
function setupToolbar(effects, state, map) {
  ...
  document.getElementById("btn-toggle-all").addEventListener("click", (e) => {
    ...
    e.target.textContent = state.showAll ? "Show all / Show one" : "Show one / Show all";
    updateVisibility(effects, state);
  });
  document.getElementById("btn-pause").addEventListener("click", (e) => {
    state.paused = !state.paused;
    e.target.textContent = state.paused ? "Resume animation" : "Pause animation";
  });
}

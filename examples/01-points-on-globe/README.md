...
```bash
cd examples/01-points-on-globe
npm install
...
npx vitest run      # 22/22 tests pass (13 projection, 6 embed lifecycle, 3 palette)
...
|---|---|---|---|
| Glow palette | HUD select | Plasma (also Spectrum / Solar / Aurora / Ice) | Uses a separate deterministic synthetic color hash; colors remain illustrative and do not encode airport categories |
| Point size × | HUD slider | 0.60 (range 0.2–3) | `uSizeMul` — scales every point's pixel size |
| Opacity | HUD slider | 0.65 (range 0.1–1) | `uOpacity` — overall glow alpha |
| Core boost | HUD slider | 0.85 (range 0–1) | `uCoreBoost` — how hard the point center is pushed toward white |
...
- **Point size and color are synthetic.** OurAirports doesn't publish traffic figures, so `src/airports.ts` derives separate stable hashes from each airport's `ident` for size and color, then applies the selected Spectrum/Solar/Aurora/Plasma/Ice ramp. The palettes are visual alternatives, not real airport categories. This produces a plausible-looking varied distribution, but it has **no relationship to actual passenger or flight volume** — don't read anything into which airports render large or which hue they receive.

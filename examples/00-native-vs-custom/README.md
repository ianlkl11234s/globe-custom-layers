...
> Status: 🔬 **Reproduced** — builds clean, 20/20 unit tests, and visually verified with a real token: both layers render on the globe at identical positions, and clicking confirms the native layer answers `queryRenderedFeatures` while the custom layer does not.
...
|---|---|---|
| Code to hug the globe | `map.addLayer({ type: "circle", ... })` — Mapbox projects it onto the sphere internally | [`globeProject.ts`](src/globeProject.ts) + [`glowPointsScene.ts`](src/glowPointsScene.ts) + [`glowLayer.ts`](src/glowLayer.ts) — 680 lines, hand-written ECEF math, ported from [1.1 Hugging the globe: Mapbox](../../docs/01-hugging-the-globe/mapbox.md) |
...
npx vitest run      # 20/20 tests pass (13 globe-math + 7 GeoJSON-conversion)
npm run build        # succeeds (vite build)
```

The unit tests cover two things in isolation, without a WebGL context:
...
Copied **unmodified** from [`examples/01-points-on-globe`](../01-points-on-globe/) (per this repo's "self-contained examples, duplication is fine" rule):

- `src/globeProject.ts`, `src/globeProject.test.ts`
- `src/glowPointsScene.ts`
- `src/glowLayer.ts`
...
- **Custom-layer visibility is "opacity 0", not a real toggle.** `CustomLayerInterface` has no visibility flag, and `glowLayer.ts` is copied unmodified on purpose. Switching to `native` mode drives the custom layer's `getOpacity()` control to `0` instead — with `THREE.AdditiveBlending`, zero alpha genuinely draws nothing, at the cost of Mapbox still calling `render()` on it every frame either way (which is also *useful* here: it's why the `projection`/`transition` readout keeps updating in every mode, not just `custom`/`both`).
...
- Every other simplification listed in [`01-points-on-globe`'s own README](../01-points-on-globe/README.md#deliberate-simplifications) (repaint throttling, single color ramp, fixed cull width, etc.) applies here too, since `glowPointsScene.ts`/`glowLayer.ts` are the same files.

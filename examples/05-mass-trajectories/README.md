...
- **A single soft gradient fade per trail, not per-object visual variety beyond hue.** Colour is a deterministic per-object hue spread. Dark mode uses additive head glow and a quadratic tail fade; light mode uses saturated ink colours and a slower tail fade so colour differences remain readable against pale land. Within each theme, every object uses the same shader math.
